...
if(_menu==0){
	...
	if(instance_exists(_inst_changeurname)){
		instance_destroy(_inst_changeurname);
	}
...
if(_menu==6){
	...
	//	_inst_instruction.text=_prefix+"{color_text `gray_light`} --- Instruction ---{space_y -1}&&{space_y 2}[Z or ENTER] - Confirm&[X or SHIFT] - Cancel&[C or CTRL] - Menu (In-game)&[F4] - Fullscreen&[Hold ESC] - Quit&When HP is 0, you lose.";
		...
		_inst_changeurname.text=_prefix+"Change Name";
		_inst_hardmode=instance_create_depth(170,210,0,text_typer);
		_inst_hardmode.text=_prefix+"Hardmode:On";
		with(text_typer){
			event_user(15);
		}
		event_user(2);
...
if(_menu==5){
	...
	//	_inst_instruction=instance_create_depth(170,40,0,text_typer);
	//	_inst_instruction.text=_prefix+"{color_text `gray_light`} --- Instruction ---{space_y -1}&&{space_y 2}[Z or ENTER] - Confirm&[X or SHIFT] - Cancel&[C or CTRL] - Menu (In-game)&[F4] - Fullscreen&[Hold ESC] - Quit&When HP is 0, you lose.";
		_inst_phase1=instance_create_depth(170,120,0,text_typer);
		_inst_phase1.text=_prefix+"Phase 1";
		_inst_phase2=instance_create_depth(170,150,0,text_typer);
		_inst_phase2.text=_prefix+"Phase 2";
		_inst_phase3=instance_create_depth(170,180,0,text_typer);
	...
	if(instance_exists(_inst_phase1)){
		instance_destroy(_inst_phase1);
	}
	if(instance_exists(_inst_phase2)){
		instance_destroy(_inst_phase2);
	}
	if(instance_exists(_inst_phase3)){
		instance_destroy(_inst_phase3);
	}
